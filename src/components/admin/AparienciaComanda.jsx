...
export default function AparienciaComanda({ camposRegistro, configComanda, setConfigComanda, onGuardar }) {
  ...
  useEffect(() => {
    setLocalConfig(configComanda || getDefaultConfig(camposRegistro));
  ...
  useEffect(() => {
    ...
    camposRegistro.forEach(c => {
      data[c.nombre] = t(`aparienciaComanda.ejemplo.${c.nombre}`) || c.nombre.toUpperCase();
    });
    data["productos"] = [
  ...
  const handleGuardar = () => {
    setConfigComanda(localConfig);
    if (onGuardar) onGuardar(localConfig);
  };
